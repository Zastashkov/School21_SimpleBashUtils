...
void check_flags(int argc, char **argv) {
    ...
    if (characteristics.empty_lines)
        flag.o = 0;

    // Making a string that we have to find
    if (!flag.e && !flag.f)
        strcat(observable_string, argv[optind++]);
//        printf("e: %d\n", flag.e);
//        printf("i: %d\n", flag.i);
//        printf("v: %d\n", flag.v);
//        printf("c: %d\n", flag.c);
...
void find_and_print(char *argv[], FILE* file) {
    regex_t regex;   // regcomp() and regexec() Variable that saves compiled regular expression
    int cflags = REG_EXTENDED;  // regcomp() Flags that add the conditions of finding a string
